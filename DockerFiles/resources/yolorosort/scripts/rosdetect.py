...
from sort.sort import * # import sort class for yolo and ros
from utils.augmentations import letterbox # for im0 to im in dataset
...
class subscriber:
    ...
    def __init__(self):
        weights = ROOT / '720p2200_results_bigSet.pt'  # model.pt path(s)
        source = ROOT / 'data/images'  # file/dir/URL/glob, 0 for webcam
        self.imgsz = (720, 1280)  # inference size (height, width) must be multiple of max stride 32
        self.conf_thres = 0.4  # confidence threshold
        self.iou_thres = 0.5  # NMS IOU threshold
        self.max_det = 200  # maximum detections per image (was 1000)
        ...
        self.sub = rospy.Subscriber('/rgb/image_raw', Image, self.callback)  # instantiate the Subscriber and Publisher
        self.pub1 = rospy.Publisher('/yolo/boundingboxes', fullBBox, queue_size = 10)
        self.pub2 = rospy.Publisher('/yolo/boundingboxes_crop', singleBBox, queue_size = 10)
        self.pub3 = rospy.Publisher('/testing', Image, queue_size = 10)
    ...
    def callback(self, data):
        ...
        fBox.seqOriginal = data.header.seq # seq from azure
        fBox.header.stamp = rospy.get_rostime() # stamp from current time not from azure
        """
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):  # loop for the small BBoxes(Crops)
                    ...
                    # for debugging
                    # cv2.imshow("smallBBox", crop) # geht nicht im docker
                    # cv2.waitKey(1)
            ...
            #float oneBox[];
            # takes the values of the sortAlgo and calculates the best bounding box match and adds it to the fBox
            for d in trackers:
            ...
            # cv2.imshow("BBox", im0)       # geht nicht im docker
            # cv2.waitKey(1)
            ...
            self.pub3.publish(bridge.cv2_to_imgmsg(im0)) # testing purpose 
            self.imageId += 1
            print(self.imageId)
